...
async def scan_file(db, file_id: str) -> None:
    ...
    if priv_key is None:
        logger.warning(
            "av_scan_endpoint is configured but TUSSHARE_ESCROW_PRIVATE_KEY is absent; cannot scan file %s",
            file_id,
        )
        return
    ...
    cursor = await db.execute(
        'SELECT chunk_index, iv, size_bytes, "offset" FROM file_chunks WHERE file_id = ? ORDER BY chunk_index',
        (file_id,),
    )
...
async def _handle_infected(db, file_id: str, original_name: str) -> None:
    ...
    try:
        await db.execute(
            "UPDATE files SET transfer_locked_at = ? WHERE id = ? AND transfer_locked_at IS NULL",
            (now, file_id),
        )
        await db.commit()
    except Exception:
